...
class Wpshop_Payment
{
	...
	$i = 13;
		...
		add_filter('init', array(&$this, 'tinkoffResult'));
    ...
    public static function sendPaymentEmailToUser($order)
    {
        ...
        wp_mail($order->client_email,
            ...
            $message,
            "Content-type: text/html; charset=UTF-8  Reply-To: {$from} From:{$from}");
    }
	...
	/**
     * handling tinkoff payment notification
    */
    public function tinkoffResult()
    {
        if (isset($_POST['TerminalKey'])) {
			$settings = get_option("wpshop.payments.tinkoff");
			if (isset($settings['terminal'])&&$_POST['TerminalKey'] ==$settings['terminal']){
				set_error_handler('exceptions_error_handler', E_ALL);
				try {
					global $wpdb;
					$order = $wpdb->get_row($wpdb->prepare("SELECT * FROM `{$wpdb->prefix}wpshop_orders` WHERE `order_id` = '%d'", array($_POST['OrderId'])));
					...
					if ($_POST['Token'] != self::getToken($_POST, $settings['secret_key'])) {
						 die('NOTOK');
					}
					...
					$status_order = (int)Wpshop_Orders::getStatus_order($order->order_id);
					if ($status_order[0]->order_status == 1) {
						self::sendPaymentEmailToUser($order);
					}

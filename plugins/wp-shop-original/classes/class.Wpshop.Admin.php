...
class Wpshop_Admin
{
	...
	public function wpshop_admin_scripts_method($hook) {
		...
		wp_enqueue_script('wpshop-admin', WPSHOP_URL .'/wp-shop-admin.js',array('jquery','jquery-ui-position','jquery-ui-tabs'));
		wp_enqueue_script('my_validate', WPSHOP_URL .'/jquery.validate.min.js', array('jquery'));
		wp_enqueue_style('wp-shop_style_main2',"/wp-content/plugins/wp-shop-original/wp-shop.css");
	...
	public function ordersAction()
	{
		...
		if (isset($_GET['act']) && $_GET['act'] == "edit")
		{
            ...
            if ($confirm==1) {
              ...
              wp_mail($email, __('Payment Confirmation','wp-shop')." #{$id} ".__('from site','wp-shop')." {$siteurl}", $message,"Content-type: text/html; charset=UTF-8
